class MealTime < ActiveRecord::Base
  ...
  def play_message
    message = "Time for #{self.name}! The time is #{Time.now.in_time_zone.strftime('%-l:%M %P')}, so how about making #{self.recipe.name}. That is not a suggestion. Do it. Or else."
    say(message)
